...
export async function sourceRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.get('/projects/:id/sources', async (request) => {
    ...
    return { success: true, data: sources, meta: { timestamp: new Date().toISOString(), requestId: request.id } };
  });
  ...
  fastify.post('/projects/:id/sources', async (request, reply) => {
    ...
    // Handle file upload or URL/note
    ...
    for await (const part of parts) {
      if (part.type === 'file') {
        ...
        if (part.fieldname === 'url') {
          await sourceService.createFromUrl(id, userId, value);
        } else if (part.fieldname === 'note') {
          await sourceService.createFromNote(id, userId, value);
        }
      }
    }
    
    reply.status(201);
    return { success: true, data: { message: 'Source created' }, meta: { timestamp: new Date().toISOString(), requestId: request.id } };
  });
